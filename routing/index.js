...
router.get('/scraper', (req,res) =>  {
  request('https://www.cnn.com/', (error, response, html) =>  { 
  const $ = cheerio.load(html);

    $('article.media').each((i,elem) =>  {
      var article = {}
      var x = $(elem)
      article.title = x.find('strong').text().trim();
      article.summary = x.find('p').text().trim();
      article.link = "https://www.cnn.com/" + x.find('a').attr('href');
